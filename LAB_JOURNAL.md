...
> 规则：每次运作（任何工作段落）结束在本文件末尾追加一节，同一文件顺写、时间正序。每节固定四问：**做了什么 / 为什么（决策依据）/ 关键数字 / 产物路径**，末尾记下一步。细节标准：**凭本文件 + records/ 能复现当天全部操作与决策**——包括弯路、报错原文关键词、被否掉的方案。写简历时以本文件（叙事）+ RESUME_EVIDENCE.md（句子装配）+ README.md 门面 / LEDGER.md 台账（状态速查）三件套为准。数字一律以 provenance 文件为最终依据。
...
## 2026-09-15（续六）EXP-029 1.63× 的最后一块：发现它是口径不对称，不是扩展失效

**做了什么**：兑现 EXP-027 §6④ 的挂账项——测"两实例共享主机资源"这一候选。设计上刻意**保持"两实例都在跑"不变，只改负载分布**：S1 单实例 600/conc64（对照）、S2 单实例 1200/conc128（关键格）、S3 单实例 1200/conc64（分离 N 与并发），8200 全程空转、不经代理。

**为什么（决策依据）**：这是 1.63× 归因链上最后一个未测变量。若把 8200 停掉再测，差里会混进"第二实例占用的内存/CPU 是否影响第一实例"这个无关变量，所以必须让两实例都活着。

**关键数字**：S1 = **11.135**（与 EXP-027 A1 的 11.140 差 **0.04%** —— 装置校验近乎完美，同时说明"第二实例在不在跑"不影响被压实例）；S2 = **13.049**；S3 = 11.103。H-A（单实例即可承载双实例负载）**被否**，H-B 成立（22.392/13.049 = 1.716×）。**但核心发现是**：把数摆到**同一每-GPU 在飞量**上比 —— 每卡 64 在飞时 2 实例 22.392 vs 2×单实例 22.207 = **100.8%**；每卡 128 在飞时 2 实例 25.297（EXP-025 实测）vs 预测 2×13.049=26.098 = **96.9%**。**所以 `1.63× = 20.87/12.807` 是拿"replica2 每卡 64 在飞"除以"colocate 单卡 128 在飞"——两个不同的操作点被放进同一个比值**；按同操作点比，第二张卡是 97–101% 线性加成，**没有可测的主机级干扰**。附带：单实例 conc64→128 使吞吐 11.10→13.05（+17.5%）而 TPOT p50 39.0→69.6 ms（+79%，破 SLO）→ 单卡 conc64 是本机这套 workload 的自然操作点（与 EXP-024 的"tp2 在 conc64 即饱和"同源）。

**产物路径**：`records/EXP-029_single_instance_capacity.md`；raw `pd_disagg/results/b1_matrix/raw/20260915T1425_exp029_{s1_600c64,s2_1200c128,s3_1200c64}_bench.{json,log}`、服务日志同前缀；不进 runs.jsonl。

**下一步**：① 四臂在**同一每卡在飞量**下的公平重测（本实验只覆盖 replica2/colocate；tp2 是单实例、pd1p1d 天生两实例，"每卡在飞量"的定义要先想清楚）；② 主机侧直接观测（CPU/PCIe 计数）以把"无主机级干扰"从推断变成实测。
