...
> **一句话结论**：**EXP-007 的 replica2@512 = 15.58 req/s 确系欠饱和**：同栈、同 seed（1099）、同 N=400，SAT_CONC 64→128 后饱和吞吐 **20.87 req/s**（+34%，远超锁定阈值 +5%）〔**N=400 口径**；EXP-025 实测 conc128@N=1200 = 22.12，+6.0%〕，gate 通过、无失败请求。补测的 colocate@512 conc128（fresh 单实例，0 前缀命中）= **12.81 req/s**（原 conc64 10.36，+24%），**512 桶扩展效率按同并发口径修正为 20.87/12.81 = 1.63×**（原 1.50×；2048/8192 桶的 1.93/1.98× 未复测，不改）。
