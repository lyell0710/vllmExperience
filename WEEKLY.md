...
| **D2：两个空缺 config 交付，kernel 两端 -3.3~-8.5%,e2e TPOT +0.8~1.2%,120 passed**（`::test_fused_moe` 单函数；全文件子集 1041 passed / 127 skipped，EXP-015 §5.1） | S3 | **EXP-015（本周日）** |

**里程碑状态**：M1(8/31)报告 v2 已定稿，提前 8 天；M2(9/1)MoE 上卡已达成。 **缺口**：全部清零——D1–D5 及 EXT-1/2 均完成；唯余用户侧两项（线上简历排雷、PR 本人提交——已提交 #54372，未合并）。 **下周（8/24–8/30）目标**：config PR 已提交 #54372（未合并）+ 跟进 review；B4 报告终稿通读； 9 月上旬投递版简历成稿（S1–S4 全部有数）；可选：AutoGPTQMoEMethod supports_eplb 上游贡献（查重后）。
