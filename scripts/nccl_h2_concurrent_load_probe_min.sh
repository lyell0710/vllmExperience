#!/bin/bash
# EXP-020 附录 B 重试（post-hoc，最小占用：1 channel / 1MB buffsize / -e 8M）：H2「并发 vLLM 负载压低 SHM 路径 allreduce」旁证。
...
ARGS="-b 1M -e 8M -f 2 -g 2 -n 20"; export NCCL_MAX_NCHANNELS=1 NCCL_BUFFSIZE=1048576
Q="index,timestamp,pcie.link.gen.current,pcie.link.width.current,clocks.sm,utilization.gpu,memory.used"
run() { # $1=tag
  local out="$HW/${STAMP}_nccl_h2min_$1.txt" pcie="$HW/${STAMP}_nccl_h2min_$1_pcie.csv"
  ...
  { echo "# provenance: env=sys sha=717b683182 cmd=\"env LD_LIBRARY_PATH=$NCCL_LIB NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=INIT $BIN -d float $ARGS\" date=$(date -u +%FT%T+00:00) gpu=\"RTX 4090 x2\" driver=$DRV tool_sha=717b683182 nccl=libnccl.so.2.28.9 exp=EXP-020-appendixB tag=$1 note=post-hoc-H2旁证-vllm双实例常驻-显存受限-最小占用重试-NCCL_MAX_NCHANNELS=1-NCCL_BUFFSIZE=1M-e8M"
...
BL="$HW/${STAMP}_nccl_h2min_loadgen_bench.log"
