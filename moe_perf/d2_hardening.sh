#!/bin/bash
# D2 PR hardening 重测：kernel A/B 3 轮交叉次序 + correctness 全量日志
# 只做 kernel 与 correctness；e2e 不重跑（PR 正文里它只作 supporting）
set -u
DIR=$(cd "$(dirname "$0")" && pwd)
ROUND=${1:-all}
STAMP=${STAMP_OVERRIDE:-$(date -u +%Y%m%dT%H%M)}
OUT=$DIR/raw/EXP-015/hardening_$STAMP
mkdir -p "$OUT"
...
run() {  # $1=ep|noep  $2=default|tuned  $3=round
  ...
  echo "# provenance: env=venvs/main sha=$SHA cmd=\"$CMD\" date=$(date -u +%Y-%m-%dT%H:%M:%SZ) gpu=\"2xRTX 4090\" driver=$DRV arm=$ARM round=$R" > "$F"
...
echo "=== hardening 开始 $(date -u +%FT%TZ)  输出: $OUT  轮次: $ROUND"
ROUNDS="1 2 3"
case "$ROUND" in all) ;; corr) ROUNDS="" ;; *) ROUNDS="$ROUND" ;; esac
for R in ${ROUNDS:-}; do
...
[ "$ROUND" = all ] || [ "$ROUND" = corr ] || { echo "=== 本次只跑 kernel 轮 $ROUND，correctness 跳过"; exit 0; }
echo "=== correctness（全量日志）"
CF="$OUT/${STAMP}_correctness_pytest.log"
CCMD="$VENV/bin/python -m pytest tests/kernels/moe/test_moe.py -q -k 'not deepseek and not fp8 and not int8 and not wna16' -x --no-header"
echo "# provenance: env=venvs/main sha=$SHA cmd=\"$CCMD\" date=$(date -u +%Y-%m-%dT%H:%M:%SZ) gpu=\"1xRTX 4090\" driver=$DRV" > "$CF"
cd /root/projects/vllm && CUDA_VISIBLE_DEVICES=0 $VENV/bin/python -m pytest \
  ...
  -x --no-header >> "$CF" 2>&1
echo "  pytest rc=$?  行数=$(wc -l < "$CF")"
tail -2 "$CF"
rmdir "$HOLD" 2>/dev/null
echo "=== D2_HARDENING_DONE $(date -u +%FT%TZ)"
