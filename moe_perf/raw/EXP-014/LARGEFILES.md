...
因运行主机即将到期，这里的原则是：**不可再生的 artifact 一定要进 git（可 push 到远端），可再生的衍生品只留本机**。`d1_nsys_moe_bs1.nsys-rep` 原先因 159 MB 超 GitHub 单文件 100 MB 硬限而只留本机；本次以 `xz -6` 压到 66.9 MB（比率 0.400）后入库，消除该不可逆风险。压缩档比在库内的 25 MB 常规上限大，属**刻意偏离**（CORE 铁律 5 的 ≤25 MB 是防仓膨胀的常规值，此处以「保全唯一不可再生证据」为先）；如需回收体积，删掉 `*.xz` 即可，其余均不受影响。
