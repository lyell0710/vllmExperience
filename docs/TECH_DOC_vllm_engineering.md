...
├── records/EXP-001…024  八节实验记录（§1 假设与阈值跑前锁定；§4 raw 指针；§6 实测/推断分开）
...
一个轮询代理 rr_proxy.py（最小实现，不做会话亲和，所以 replica2 数字是这一族方案的下限）；conc64 口径下 512 桶饱和 15.58 的 1.50× 扩展曾是疑点，**EXP-023/024 复测（conc128 四臂统一）确系欠饱和：replica2 20.87 / colocate 12.81 → 扩展效率 1.63×**（附录 D）。剩余缺口：conc128 下 replica2 的 TPOT p50 36.8 ms 仍 < SLO，未封顶（EXP-025 已推到 conc256 25.30 req/s 仍未见平台）；**rr_proxy 转发开销已实测 ≈2%（EXP-027，两臂并发形态对齐，两次 B 臂差 0.19%），缺口不在转发层**——候选收窄为「两实例共享主机资源」与「colocate 分母本身是过饱和态」。
...
| 8 | "记录 EXP-001~017 / 17 份"（HANDOFF / README / REPORT / 怎么读） | 文档不一致 | 📝 已同步为 24 |
...
## 附录 D 本文写作期间补跑的六个实验（2026-09-15）

> 六个都是附录 A 里"计划过但没跑"的条目（前四个 + 后续的 EXP-024 口径补齐与 EXP-020 附录 C 的 H3 长跑）。数字照抄各记录 §5；记录与 raw 路径在 LEDGER 台账。
...
### EXP-024《512 桶四臂统一到 conc128 口径》— 完成，口径补齐

为让 fig7 的 512 桶四臂同口径（避免一个桶里两臂 conc128、两臂 conc64），补测 tp2 与 pd1p1d：

| 臂 | conc64 | **conc128** | Δ | TTFT p50 ms | TPOT p50 ms | 功率峰 W | 节流 |
|---|---:|---:|---:|---:|---:|---:|---|
| colocate | 10.36 | **12.81** | +23.6% | 835 | 67.1 | 450.1 | 0x1+0x4 |
| replica2 | 15.58 | **20.87** | +34.0% | 771 | 36.8 | 450.3 | 0x1+0x4 |
| tp2 | 12.31 | **12.30** | −0.1% | 1046 | 70.3 | **326.3** | 仅 0x1 |
| pd1p1d | 7.84 | **8.15** | +4.0% | **12669** | **18.97** | 453.5 | 0x1+0x4 |

512 扩展效率（同口径）**1.63×**（原 1.50×）。两个结论值得记住：

- **tp2 与 PD 根本不是"欠饱和"**：tp2 在 conc64 就饱和（12.31→12.30），pd1p1d 在 conc64 就撞传输墙——`bytes/wall` = 0.230 / 0.239 GB/s，上限模型 `0.2393 ÷ 0.02936 = 8.15` 与实测 8.152 精确吻合（conc64 同样：0.2302 ÷ 0.02936 = 7.84 = 实测）。只有 colocate/replica2 真欠饱和。
- **tp2 是四臂唯一不触发功率帽的**（功率峰 326 W / 无 0x4）：TP2 把 prefill 摊到两卡，单卡功耗减半——这是 §1.1 功率帽机制的正面对照。

**方法学（新增一条口径陷阱）**：并发 >1 时 `bytes / ΣxferDuration` 不再是吞吐——它把并发重叠的每请求时长累加了（conc128 下给出 0.0026 GB/s，比真实聚合速率低两个数量级）。EXP-006 在并发 1 下 `Σxfer ≈ wall`，所以那个等式只**在串行时**成立，与"两卡时 busbw 恰等于 algbw"是同类陷阱。聚合速率一律用 `bytes / wall_time`。

### EXP-020 附录 C《H3 长跑探针》— 60 轮零塌陷（按锁定阈值），但低平台态现象成立

SHM 路径 60 轮大消息扫描（float/half 交替、200ms PCIe/SM/mem/pstate/power 采样）：

- 平台 min 3.23 / max 9.23 / mean 7.37 GB/s；**按跑前锁定的 `<3.0 判塌陷`：0 次 → 判定 C**（频率 <1.7%）。
- **post-hoc**：第 43 轮平台 3.23 GB/s 未达线，但**逐 size 曲线从 1M 起平坦**（2.98→3.64），与 EXP-021 那次 2.15 的塌陷、EXP-002 的 1.78 是同一曲线家族（正常轮是 5.11→7.59 的爬升+平台形）。按"形状"判据，低平台态频率 1/60，与 EXP-021 的 1/28 合并 **2/88 ≈ 2.3%**，且**两次都在 Gen4 x16、SM 2835 MHz、mem 10501 MHz 满血下**发生。
- **对 1.78 归因**：观测到的低平台态分位是 2.15 与 3.23，**尚未见到 ≤2.3 以下**；而 Socket 档稳定复现 1.51–1.70。1.78 落在 Socket 档略上、低平台态之下 → **证据天平向 Socket 路径倾斜，但仍不追认**（样本量对"更低平台态存在"给不出否定）。H3 机理不是 PCIe 未升频，候选转向主机侧或未知。R0-1 维持"停用"。

### 这四个实验改变了什么

- **NCCL 带宽**：从"两个数打架、无法定因"到"两条路径各有数字、SHM 还有一个塌陷态"——面试口径从"待复核"升级为"带路径引用"，但追认动作归用户。
- **config 调优**：负结论的适用范围缩小到 M≤256；大 M 段是明确正收益。
- **四臂表**：512 桶从"疑欠饱和"收敛为"两臂真欠饱和（colocate +23.6% / replica2 +34.0%）、两臂早已到顶（tp2 饱和 / PD 撞墙）"，排序不变、且 tp2 与 PD 的天花板第一次被定量钉住；扩展效率同口径 1.63×。
