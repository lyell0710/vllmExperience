...
一个轮询代理 rr_proxy.py（最小实现，不做会话亲和，所以 replica2 数字是这一族方案的下限）；conc64 口径下 512 桶饱和 15.58 的 1.50× 扩展曾是疑点，**EXP-023/024 复测（conc128 四臂统一）确系欠饱和：replica2 20.87 / colocate 12.81 → 扩展效率 1.63×**（附录 D）。剩余缺口：conc128 下 replica2 的 TPOT p50 36.8 ms 仍 < SLO，未封顶；rr_proxy 开销未拆分。
